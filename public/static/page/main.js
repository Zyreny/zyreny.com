...
function loadNav(btns) {
    ...
    const btmNavHTML = btns.map(btn => `<a href="${btn.href}"><li><img src="https://zyreny.com/img/nav/${btn.id}.svg" alt="${btn.name}" loading="lazy">${btn.name}</li></a>`).join("");

    nav.innerHTML = `
    <header>
        <h1>Zyreny</h1>
        <a href="/"><img src="https://zyreny.com/img/logo/logo_full.svg" alt="Zyreny" loading="lazy"></a>
